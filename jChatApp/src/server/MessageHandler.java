	...
	public void run() {
		...
		while (true) {
			...
			while (it.hasNext()) {
				...
				Server.LOG.info("MessageHandler: Action: Processing Message, MessageID: " + message.getMessageID()
				...
				case 1:
					sendMessage(message);
					Server.LOG.info("MessageHandler: Action: Sending, MessageID: " + message.getMessageID()
							+ ", Content: " + message.getContent());
					break;
				// Message Type 2 User is joining (content is user name)
				case 2:
					userAdd(message);
					Server.LOG.info("MessageHandler: Action: Adding User, MessageID: " + message.getMessageID()
							+ ", Username: " + message.getContent());
					break;
				// Message Type 3 User left (content is user name)
				case 3:
					userRemove(message);
					Server.LOG.info("MessageHandler: Action: Removing User, MessageID: " + message.getMessageID()
							+ ", Username: " + message.getContent());
					break;
				...
				case 9:
					disconnect(message);
					Server.LOG.info("MessageHandler: Action: User Disconnect, MessageID: " + message.getMessageID()
							+ ", Username: " + message.getContent());
					break;
				default:
					Server.LOG.severe("MessageHandler: Error: Message with unknown Message Type recived");
					break;
				}
			}
		}
	...
	public void sendMessage(Message message) {
		...
		while (it.hasNext()) {
			try {
				...
				Server.LOG.warning(
						"MessageHandler: Error: Could not send message to " + it.toString() + " with exeption: " + ex);
			}
		}
	}
}